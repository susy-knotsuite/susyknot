...
/**
 * Update packages that depend on polc to the latest version
 *
 * @param dryRun {Boolean} Do a dry run and only return the packages that would be updated
 * @returns packageNames {String[]} Returns the packages names that depend on polc
 **/
function updatePackages(dryRun = false) {
  ...
  return packages
    .map(packageName => {
      ...
      // operate if the polc dependency exists
      if (packageJson.dependencies && packageJson.dependencies["polc"]) {
        if (dryRun) return packageName;
        return updatePackage(packageName, packagePath, false);
      } else if (
        packageJson.devDependencies &&
        packageJson.devDependencies["polc"]
      ) {
        if (dryRun) return packageName;
 ...
 * @param dev {Boolean} True if polc is a dev dependency
...
function updatePackage(packageName, packagePath, dev) {
  ...
  execSync(`npm install polc@latest ${dev ? "--save-dev" : "--save"}`, {
    cwd: packagePath
  });
  return packageName;
}

// Get the packages that have a polc dependency
const polcPackageDeps = updatePackages(true);

console.log("Bumping polynomial version for the following packages:");
console.log(polcPackageDeps);
...
rl.question("\nContinue: (Y/n)\n", answer => {
  ...
  rl.close();
  updatePackages();
  console.log(`Updated ${polcPackageDeps.length} packages to polc@latest`);
  process.exit(0);
});
