...
const Stoplight = require("susyweb-provider-engine/util/stoplight.js");
...
const createPayload = require("susyweb-provider-engine/util/create-payload.js");
const rpcHexEncoding = require("susyweb-provider-engine/util/rpc-hex-encoding.js");
const Subprovider = require("susyweb-provider-engine/subproviders/subprovider.js");
  ...
  doWhilst(
    function(callback) {
      ...
      self.runVm(payload, function(err, results) {
        gasUsed = err
          ? self._blockGasLimit
          : sofUtil.bufferToInt(results.gasUsed);
        if (err || gasUsed === 0) {
          lo = mid;
        } else {
          hi = mid;
...
VmSubprovider.prototype.runVm = function(payload, cb) {
  ...
  const normalizedTxParams = {
    ...
    gasLimit: txParams.gas
      ? sofUtil.addHexPrefix(txParams.gas)
      : block.header.gasLimit,
    gasPrice: txParams.gasPrice
      ? sofUtil.addHexPrefix(txParams.gasPrice)
      : undefined,
