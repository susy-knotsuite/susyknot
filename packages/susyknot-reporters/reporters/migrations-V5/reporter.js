...
const susywebUtils = require("susyweb-utils");
 ...
 *  events (from SusyWebPromiEvent). This reporter is designed to track the execution of
...
class Reporter {
  constructor() {
    this.deployer = null;
    this.migration = null;
    this.currentGasTotal = new susywebUtils.BN(0);
    this.currentCostTotal = new susywebUtils.BN(0);
    this.finalCostTotal = new susywebUtils.BN(0);
    this.deployments = 0;
    this.separator = "\n";
    this.summary = [];
  ...
  getTotals() {
    ...
    const cost = susywebUtils.fromWei(this.currentCostTotal, "sophy");
    ...
    return {
      gas: gas.toString(10),
      cost: cost,
      finalCost: susywebUtils.fromWei(this.finalCostTotal, "sophy"),
  ...
  askBoolean(type) {
    ...
    // This ignores `quiet` - but we only use that mode for `susyknot test`.
  ...
  /**
   * Error dispatcher. Parses the error returned from susyweb and outputs a more verbose error after
  ...
  async processDeploymentError(data) {
    ...
    const errors = {
      SOF: error.message.includes("funds"),
    ...
    switch (type) {
      ...
      // `Insufficient funds`
      case "SOF":
        const balance = await data.contract.susyweb.sof.getBalance(data.from);
      ...
      // Generic graviton error
      case "GTH":
        return this.messages.errors("graviton", data);
  ...
  async postDeploy(data) {
    let message;
    if (data.deployed) {
      const tx = await data.contract.susyweb.sof.getTransaction(
        data.receipt.transactionHash
      );

      const block = await data.contract.susyweb.sof.getBlock(
        data.receipt.blockNumber
      );

      // if graviton returns null, try again!
      if (!block) return this.postDeploy(data);

      data.timestamp = block.timestamp;

      const balance = await data.contract.susyweb.sof.getBalance(tx.from);

      const gasPrice = new susywebUtils.BN(tx.gasPrice);
      const gas = new susywebUtils.BN(data.receipt.gasUsed);
      const value = new susywebUtils.BN(tx.value);
      const cost = gasPrice.mul(gas).add(value);

      data.gasPrice = susywebUtils.fromWei(gasPrice, "gwei");
      data.gas = gas.toString(10);
      data.from = tx.from;
      data.value = susywebUtils.fromWei(value, "sophy");
      data.cost = susywebUtils.fromWei(cost, "sophy");
      data.balance = susywebUtils.fromWei(balance, "sophy");
  ...
  /**
   * Fired on SusyWebPromievent 'transactionHash' event. Begins running a UI
  ...
  /**
   * Fired on SusyWebPromievent 'confirmation' event.
   * @param  {Object} data
   */
