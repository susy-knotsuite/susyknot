...
import { Abi } from "susyknot-contract-schema/spec";
...
export namespace Contexts {
  ...
  //really, we should import the ABI spec from susyknot-contract-schema;
        ...
        ...abi.filter(
          ...
          abiEntry => {
            ...
            if(signature === undefined) {
              ...
              //Notice the type cosrcion -- susyweb and our schema describe things a little
  ...
  export function matchContext(context: Context, givenBinary: string): boolean {
    ...
    if (
      ...
      lengthDifference < 0 ||
      lengthDifference % (2 * SVM.WORD_SIZE) !== 0
    ) {
      return false;
    }
    for (let i = 0; i < binary.length; i++) {
      ...
      //note that we need to compare case-insensitive, since Polynomial will
  ...
  export function normalizeContexts(contexts: Contexts): Contexts {
    ...
    //them.  Once we have susyknot-db, this step should largely go away.
    ...
    const fillerLength = 2 * SVM.ADDRESS_SIZE;
    ...
    //<40, while also catching the Polynomial compiler's link reference format
    //as well as Susyknot's.  Hooray!
