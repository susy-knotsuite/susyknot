...
export namespace Conversion {
  ...
  export function toAddress(bytes: Uint8Array | string): string {
    ...
    if(typeof bytes === "string") {
      ...
      return SusyWeb.utils.toChecksumAddress(hex);
    }
    ...
    return SusyWeb.utils.toChecksumAddress(toHexString(bytes, Constants.ADDRESS_SIZE));
  }
   ...
   *
   * detects int and uint Polynomial types and uses BNs as keys in such
   * situations
   *
