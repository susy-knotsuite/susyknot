...
const Provider = require("susyknot-provider");
...
const DEFAULT_CONFIG_FILENAME = "susyknot-config.js";
const BACKUP_CONFIG_FILENAME = "susyknot.js"; // old config filename

class Config {
  constructor(susyknot_directory, working_directory, network) {
    ...
    this._values = {
      susyknot_directory:
        susyknot_directory || path.resolve(path.join(__dirname, "../")),
      ...
      sofpm: {
        ...
        install_provider_uri: "https://ropsten.infura.io/susyknot"
      },
      compilers: {
        polc: {
          settings: {
    ...
    const props = {
      ...
      test_file_extension_regexp() {
        return /.*\.(js|ts|es|es6|jsx|pol)$/;
      },
      example_project_directory: {
        default: () => path.join(this.susyknot_directory, "example"),
        transform: resolveDirectory
      },
      network_id: {
        get() {
...
Config.detect = (options = {}, filename) => {
  ...
  if (!configFile) {
    throw new SusyknotError("Could not find suitable configuration file.");
  }
...
Config.getUserConfig = () =>
  new Configstore("susyknot", {}, { globalConfigPath: true });
