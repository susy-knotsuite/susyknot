...
/**
 *  This API is consumed by `susyknot-core` at the `migrate` and `test` commands via
...
const Migrate = {
  ...
  lastCompletedMigration: async function(options) {
    ...
    const migrationsOnChain = async (migrationsAddress, callback) => {
      return (
        (await Migrations.susyweb.sof.getCode(migrationsAddress, callback)) !==
        "0x"
      );
    };

    // Two possible Migrations.pol's (lintable/unlintable)
