...
const Require = require("susyknot-require");
const Emittery = require("emittery");
const SusyWebShim = require("susyknot-interface-adapter").SusyWebShim;

const ResolverIntsrcept = require("./resolverintsrcept");
...
class Migration {
   ...
   * @param  {Object}   context  susyweb
   * @param  {Object}   deployer susyknot module
   * @param  {Object}   resolver susyknot module
   */
  async _load(options, context, deployer, resolver) {
    // Load assets and run `execute`
    const accounts = await context.susyweb.sof.getAccounts();
  ...
  async _deploy(options, deployer, resolver, migrateFn) {
    try {
      ...
      if (this.isLast) {
        ...
        // our fork just swallows errors from sof-block-tracking
  ...
  // ------------------------------------- Private -------------------------------------------------
  /**
   * Runs the legacy migration sequence by used in Susyknot v4,
   * but continues to use the concurrent Deployer and SusyWeb provider
   * @param  {Object}   options  config and command-line
   */
  async runLegacyMigrations(options) {
    ...
    try {
      ...
      const Migrations = resolver.require("./Migrations.pol");
  ...
  async run(options) {
    ...
    const { susyweb, resolver, context, deployer } = this.prepareForMigrations(
      options
    );
  ...
  prepareForMigrations(options) {
    ...
    const resolver = new ResolverIntsrcept(options.resolver);

    // Initial context.
    const context = { susyweb };
    ...
    return { logger, susyweb, resolver, context, deployer };
  }
}
