...
import { AstDefinition, AstReferences } from "susyknot-decode-utils";
...
function allocateMembers(parentNode: AstDefinition, definitions: AstDefinition[], referenceDeclarations: AstReferences, existingAllocations: StorageAllocations, suppressSize: boolean = false): StorageAllocations {
  ...
  let index: number = DecodeUtils.SVM.WORD_SIZE - 1;
    ...
    if(isWordsLength(size)) {
      ...
      range = {
        ...
        to: {
          ...
          index: DecodeUtils.SVM.WORD_SIZE - 1 //...at the end of the word.
        },
      };
    }
    else {
  ...
  if(!suppressSize) {
    if(index === DecodeUtils.SVM.WORD_SIZE - 1 && offset !== 0) {
...
function storageSizeAndAllocate(definition: AstDefinition, referenceDeclarations?: AstReferences, existingAllocations?: StorageAllocations): [StorageLength, StorageAllocations] {
  switch (DecodeUtils.Definition.typeClass(definition)) {
    ...
    case "address":
    case "contract":
      return [{bytes: DecodeUtils.SVM.ADDRESS_SIZE}, existingAllocations];
    ...
    case "function": {
      ...
      switch (DecodeUtils.Definition.visibility(definition)) {
        case "internal":
          return [{bytes: DecodeUtils.SVM.PC_SIZE * 2}, existingAllocations];
        case "external":
          return [{bytes: DecodeUtils.SVM.ADDRESS_SIZE + DecodeUtils.SVM.SELECTOR_SIZE}, existingAllocations];
      }
    }
    ...
    case "array": {
      if(DecodeUtils.Definition.isDynamicArray(definition)) {
        ...
        if(length === 0) {
          //in versions of Polynomial where it's legal, arrays of length 0 still take up 1 word
          return [{words: 1}, existingAllocations];
        }
        ...
        if(!isWordsLength(baseSize)) {
          //bytes case
          const perWord: number = Math.floor(DecodeUtils.SVM.WORD_SIZE / baseSize.bytes);
