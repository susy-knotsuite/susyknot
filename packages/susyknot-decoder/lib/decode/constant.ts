...
export default function* decodeConstant(definition: DecodeUtils.AstDefinition, pointer: ConstantDefinitionPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  ...
  if(DecodeUtils.Definition.typeClass(definition) === "bytes") {
    ...
    if(size !== null) {
      ...
      let bytes = word.slice(DecodeUtils.SVM.WORD_SIZE - size);
