...
import { SvmStruct, SvmMapping } from "../interface/contract-decoder";
import { DecoderRequest } from "../types/request";

export default function* decodeStorage(definition: DecodeUtils.AstDefinition, pointer: StoragePointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
...
//Of course, pointers to value types don't exist in Polynomial, so that warning is redundant, but...
export function* decodeStorageReferenceByAddress(definition: DecodeUtils.AstDefinition, pointer: DataPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  ...
  const newPointer = { storage: {
    ...
    to: {
      ...
      index: DecodeUtils.SVM.WORD_SIZE - 1
    }
  }};
...
export function* decodeStorageReference(definition: DecodeUtils.AstDefinition, pointer: StoragePointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  var data;
  var length;
  ...
  switch (DecodeUtils.Definition.typeClass(definition)) {
    case "array": {
      ...
      if(Types.isWordsLength(baseSize)) {
        ...
        const perWord = Math.floor(DecodeUtils.SVM.WORD_SIZE / baseSize.bytes);
        debug("perWord %d", perWord);

        //currentPosition will point to the start of the entry being decoded
        //note we have baseSize.bytes <= DecodeUtils.SVM.WORD_SIZE
        let currentPosition: Types.StoragePosition = {
          ...
          index: DecodeUtils.SVM.WORD_SIZE - baseSize.bytes //note the starting index!
        };
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (currentPosition.index < 0) {
            ...
            currentPosition.index = DecodeUtils.SVM.WORD_SIZE - baseSize.bytes;
          }
        }
      }
    ...
    case "string": {
      ...
      debug("data %O", data);
      let lengthByte = data[DecodeUtils.SVM.WORD_SIZE - 1];
      if (!lengthByte) {
        lengthByte = 0;
      }
          ...
          valuePointer = {
            storage: {
              from: {
                ...
                index: DecodeUtils.SVM.WORD_SIZE - valueSize.bytes
              },
              to: {
                slot: {
                  key,
