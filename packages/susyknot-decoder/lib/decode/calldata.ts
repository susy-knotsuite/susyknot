...
export default function* decodeCalldata(definition: DecodeUtils.AstDefinition, pointer: CalldataPointer, info: SvmInfo, base: number = 0): IterableIterator<any | DecoderRequest> {
...
export function* decodeCalldataReferenceByAddress(definition: DecodeUtils.AstDefinition, pointer: DataPointer, info: SvmInfo, base: number = 0): IterableIterator<any | DecoderRequest> {
  const { state } = info;
  debug("pointer %o", pointer);
  ...
  switch (DecodeUtils.Definition.typeClass(definition)) {
    ...
    case "string":
      length = DecodeUtils.Conversion.toBN(yield* read({
        calldata: { start: startPosition, length: DecodeUtils.SVM.WORD_SIZE}
      }, state)).toNumber(); //initial word contains length

      let childPointer: CalldataPointer = {
        calldata: { start: startPosition + DecodeUtils.SVM.WORD_SIZE, length }
      }
    ...
    case "array":
      ...
      if (DecodeUtils.Definition.isDynamicArray(definition)) {
        ...
        startPosition += DecodeUtils.SVM.WORD_SIZE; //increment startPosition to
      ...
      //then size must be SVM.WORD_SIZE
...
export function* decodeCalldataReferenceStatic(definition: DecodeUtils.AstDefinition, pointer: CalldataPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  const { state } = info;
  debug("static");
  debug("pointer %o", pointer);
...
function* decodeCalldataStructByPosition(definition: DecodeUtils.AstDefinition, startPosition: number, info: SvmInfo): IterableIterator<any | DecoderRequest> {
