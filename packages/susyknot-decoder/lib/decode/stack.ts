...
export default function* decodeStack(definition: DecodeUtils.AstDefinition, pointer: StackPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
...
export function* decodeLiteral(definition: DecodeUtils.AstDefinition, pointer: StackLiteralPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  ...
  if(DecodeUtils.Definition.isReference(definition)
    ...
    if(DecodeUtils.Definition.typeClass(definition) === "string" ||
      ...
      let start = DecodeUtils.Conversion.toBN(pointer.literal.slice(0, DecodeUtils.SVM.WORD_SIZE)).toNumber();
      let length = DecodeUtils.Conversion.toBN(pointer.literal.slice(DecodeUtils.SVM.WORD_SIZE)).toNumber();
    ...
    if(DecodeUtils.Definition.isDynamicArray(definition)) {
      ...
      //from calldata
      let locationOnly = pointer.literal.slice(0, DecodeUtils.SVM.WORD_SIZE);
      ...
      return yield* decodeCalldataReferenceByAddress(definition, {literal: locationOnly}, info, -DecodeUtils.SVM.WORD_SIZE);
    }
    else {
  ...
  if(DecodeUtils.Definition.typeClass(definition) === "function"
    ...
    let address = pointer.literal.slice(0, DecodeUtils.SVM.WORD_SIZE);
    let selector = pointer.literal.slice(-DecodeUtils.SVM.SELECTOR_SIZE);
