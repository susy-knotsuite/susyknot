...
export default function* decodeMemory(definition: DecodeUtils.AstDefinition, pointer: MemoryPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
...
export function* decodeMemoryReferenceByAddress(definition: DecodeUtils.AstDefinition, pointer: DataPointer, info: SvmInfo): IterableIterator<any | DecoderRequest> {
  const { state } = info;
  ...
  switch (DecodeUtils.Definition.typeClass(definition)) {
    ...
    case "string":
      length = DecodeUtils.Conversion.toBN(yield* read({
        memory: { start: startPosition, length: DecodeUtils.SVM.WORD_SIZE}
      }, state)).toNumber(); //initial word contains length

      let childPointer: MemoryPointer = {
        memory: { start: startPosition + DecodeUtils.SVM.WORD_SIZE, length }
      }
    ...
    case "array":
      ...
      for(let index = 0; index < length; index++) {
        decodedChildren.push(yield* decodeMemory(
          baseDefinition,
          { memory: {
            start: startPosition + index * DecodeUtils.SVM.WORD_SIZE,
            length: DecodeUtils.SVM.WORD_SIZE
          }},
          info));
      }
