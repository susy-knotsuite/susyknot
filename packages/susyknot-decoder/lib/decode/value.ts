...
import { SvmEnum } from "../interface/contract-decoder";
import { DecoderRequest } from "../types/request";

export default function* decodeValue(definition: DecodeUtils.AstDefinition, pointer: DataPointer, info: SvmInfo): IterableIterator<undefined | boolean | BN | string | DecoderRequest | Uint8Array> {
  ...
  switch (DecodeUtils.Definition.typeClass(definition)) {
    ...
    case "function":
      switch (DecodeUtils.Definition.visibility(definition)) {
        case "external":
          let address = bytes.slice(0, DecodeUtils.SVM.ADDRESS_SIZE);
          let selector = bytes.slice(DecodeUtils.SVM.ADDRESS_SIZE, DecodeUtils.SVM.ADDRESS_SIZE + DecodeUtils.SVM.SELECTOR_SIZE);
          return yield* decodeExternalFunction(address, selector, info);
        case "internal":
          let pc: Uint8Array;
          if(info.currentContext.isConstructor) {
            //get 2nd-to-last 4 bytes
            pc = bytes.slice(-DecodeUtils.SVM.PC_SIZE * 2, -DecodeUtils.SVM.PC_SIZE);
          }
          else {
            //get last 4 bytes
            pc = bytes.slice(-DecodeUtils.SVM.PC_SIZE);
          }
...
export function* decodeContract(addressBytes: Uint8Array, info: SvmInfo): IterableIterator<string | DecoderRequest | Uint8Array> {
...
export function* decodeExternalFunction(addressBytes: Uint8Array, selectorBytes: Uint8Array, info: SvmInfo): IterableIterator<string | DecoderRequest | Uint8Array> {
...
export function decodeInternalFunction(pcBytes: Uint8Array, info: SvmInfo): string | undefined {
