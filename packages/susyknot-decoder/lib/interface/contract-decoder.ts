...
import { Definition as DefinitionUtils, SVM, AstDefinition, AstReferences } from "susyknot-decode-utils";
import { BlockType, Transaction } from "susyweb/sof/types";
import { EventLog, Log } from "susyweb/types";
...
export interface SvmMapping {
  ...
  valueType: string;
  members: {
    [key: string]: SvmVariable;
  };
};

export interface SvmStruct {
  name: string;
  type: string;
  members: {
...
export interface SvmEnum {
  type: string;
  value: string;
};

type SvmVariable = BN | string | boolean | SvmMapping | SvmStruct | SvmEnum;
...
export default class SusyknotContractDecoder extends AsyncEventEmitter {
  private susyweb: SusyWeb;
  ...
  public async init(): Promise<void> {
    this.contractNetwork = (await this.susyweb.sof.net.getId()).toString();
    ...
    this.contractCode = await this.susyweb.sof.getCode(this.contractAddress);
  }
  ...
  public async state(block: BlockType = "latest"): Promise<ContractState | undefined> {
    let blockNumber = typeof block === "number"
      ? block
      : (await this.susyweb.sof.getBlock(block)).number;

    let result: ContractState = {
      name: this.contract.contractName,
      code: this.contractCode,
      balance: new BN(await this.susyweb.sof.getBalance(this.contractAddress, blockNumber)),
      nonce: new BN(await this.susyweb.sof.getTransactionCount(this.contractAddress, blockNumber)),
      variables: {}
    };
  ...
  private async getStorage(address: string, slot: BN, block: number): Promise<Uint8Array> {
    ...
    let word = DecodeUtils.Conversion.toBytes(
      await this.susyweb.sof.getStorageAt(
        address,
        slot,
        block
      ),
      DecodeUtils.SVM.WORD_SIZE
    );
  ...
  private async getCode(address: string, block: number): Promise<Uint8Array> {
    ...
    let code = DecodeUtils.Conversion.toBytes(
      await this.susyweb.sof.getCode(
        address,
        block
      )
    );
  ...
  public async events(name: string | null = null, block: BlockType = "latest"): Promise<ContractEvent[]> {
    const susywebContract = new this.susyweb.sof.Contract(this.contract.abi, this.contractAddress);
    const events = await susywebContract.getPastEvents(name, {
      fromBlock: block,
      toBlock: block
    });
  ...
  public onEvent(name: string, callback: Function): void {
    //this.susyweb.sof.subscribe(name);
  }
  ...
  private constructSlot(variable: number | string, ...indices: any[]): [Slot | undefined , AstDefinition | undefined] {
    ...
    switch(DefinitionUtils.typeClass(parentDefinition)) {
      ...
      case "mapping":
        ...
        switch(DefinitionUtils.typeClass(keyDefinition)) {
          ...
          case "bytes":
            ...
            break;
          case "address":
            index = SusyWeb.utils.toChecksumAddress(rawIndex);
            break;
          case "int":
          case "uint":
            if(rawIndex instanceof BN) {
      ...
      case "struct":
        ...
        slot = {
          path: parentSlot,
          //need type cosrcion here -- we know structs don't contain constants but the compiler doesn't
