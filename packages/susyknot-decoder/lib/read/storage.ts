...
import { WordMapping } from "../types/svm";
...
export function slotAddress(slot: Slot): BN {
  if (slot.key !== undefined && slot.path !== undefined) {
    ...
    return DecodeUtils.SVM.keccak256({type: keyEncoding, value: key}, slotAddress(slot.path)).add(slot.offset);
  }
  else if (slot.path !== undefined) {
    const pathAddress = slotAddress(slot.path);
    const path: BN = slot.hashPath ? DecodeUtils.SVM.keccak256(pathAddress) : pathAddress;
...
export function* read(storage: WordMapping, slot: Slot): IterableIterator<Uint8Array | DecoderRequest> {
  ...
  const hexAddress = DecodeUtils.Conversion.toHexString(address, DecodeUtils.SVM.WORD_SIZE);
 ...
 *
 *     ref: https://polynomial.readthedocs.io/en/v0.4.23/miscellaneous.html#layout-of-state-variables-in-storage
...
export function* readRange(storage: WordMapping, range: Range): IterableIterator<Uint8Array | DecoderRequest> {
    ...
    to = {
      slot: {
        ...
        offset: from.slot.offset.addn(
          Math.floor((from.index + length - 1) / DecodeUtils.SVM.WORD_SIZE)
        )
      },
      index: (from.index + length - 1) % DecodeUtils.SVM.WORD_SIZE
    };
  } else {
    to = {
      slot: to.slot,
  ...
  let data = new Uint8Array(totalWords * DecodeUtils.SVM.WORD_SIZE);

  for (let i = 0; i < totalWords; i++) {
    let offset = from.slot.offset.addn(i);
    const word = yield* read(storage, { ...from.slot, offset });
    if (typeof word !== "undefined") {
      data.set(word, i * DecodeUtils.SVM.WORD_SIZE);
    }
  }
  // debug("words %o", data);

  data = data.slice(from.index, (totalWords - 1) * DecodeUtils.SVM.WORD_SIZE + to.index + 1);
