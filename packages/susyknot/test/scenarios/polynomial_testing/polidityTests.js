...
describe("Polynomial Tests", function() {
  ...
  /**
   * Installs a bare susyknot project and deposits a polynomial test target
   * in the test directory.
   * @param  {Function} done callback
   * @param  {String}   file Polynomial test target
   */
  ...
  describe("test with balance", function() {
    before(function(done) {
      this.timeout(5000);
      initSandbox(done, "TestWithBalance.pol");
    });
  ...
  describe("tests failing", function() {
    before(function(done) {
      this.timeout(5000);
      initSandbox(done, "TestFailures.pol");
    });
