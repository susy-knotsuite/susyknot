...
describe("susyknot publish", function() {
  var config;
  var project = path.join(__dirname, '../../sources/sofpm');
  ...
  it.skip("Can locate all the sources to publish", function(done) {
    ...
    CommandRunner.run("compile", config, function(err) {
      ...
      assert(fs.existsSync(path.join(config.contracts_build_directory, "SIP20.json")));
