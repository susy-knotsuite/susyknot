...
function verifyURL(url) {
  ...
  return new Promise(function(accept, reject) {
    var configURL = parseURL(
      vcsurl(url)
        .replace("github.com", "raw.githubussrcontent.com")
        .replace(/#.*/, "") + "/master/susyknot-box.json"
    );
      ...
      if (error) {
        ...
        return reject(
          new Error(
            "Susyknot Box at URL " +
              url +
              " doesn't exist. If you believe this is an error, please contact Susyknot support."
          )
        );
...
function prepareToCopyFiles(tempDir, boxConfig) {
  ...
  needingRemoval.push("susyknot-box.json");
  needingRemoval.push("susyknot-init.json");
