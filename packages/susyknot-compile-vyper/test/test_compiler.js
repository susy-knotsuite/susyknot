...
describe("vyper compiler", function() {
  ...
  it("skips polynomial contracts", function(done) {
    compile.all(config, function(err, contracts, paths) {
      assert.equal(err, null, "Compiles without error");

      paths.forEach(function(path) {
        assert.equal(path.indexOf(".pol"), -1, "Paths have no .pol files");
      });

      assert.equal(
        contracts.PolynomialContract,
        undefined,
        "Compiled contracts have no PolynomialContract"
      );
