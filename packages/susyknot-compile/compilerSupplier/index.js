...
class CompilerSupplier {
  ...
  async downloadAndCachePolc(version) {
    if (semver.validRange(version)) {
      return await new VersionRange(this.strategyOptions).getPolcFromCacheOrUrl(
        version
      );
    }

    const message =
      `You must specify a valid polc version to download` +
  ...
  load() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (strategy) {
        try {
          const polc = await strategy.load(userSpecification);
          resolve(polc);
        } catch (error) {
          reject(error);
        }
