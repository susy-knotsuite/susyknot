...
class LoadingStrategy {
  ...
  errors(kind, input, error) {
    const info = "Run `susyknot compile --list` to see available versions.";
    ...
    const kinds = {
      ...
      noVersion:
        ...
        `build in the susyknot config. ${info}`,
      noRequest:
      ...
      noDocker:
        "You are trying to run dockerized polc, but docker is not installed.",
      noImage:
        "Please pull " +
        input +
        " from docker before trying to compile with it.",
      noNative: "Could not execute local polc binary: " + error,
      noString:
        "`compilers.polc.version` option must be a string specifying:\n" +
        "   - a path to a locally installed polcjs\n" +
        "   - a polc version or range (ex: '0.4.22' or '^0.5.0')\n" +
        "   - a docker image name (ex: 'stable')\n" +
        "   - 'native' to use natively installed polc\n" +
        "Received: " +
        input +
        " instead."
    };
  ...
  /**
   * Cleans up error listeners set (by polc?) when requiring it. (This code inherited from
