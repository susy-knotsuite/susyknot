...
class Docker extends LoadingStrategy {
  async load() {
    const versionString = await this.validateAndGetPolcVersion();
    const command =
      "docker run -i sophon/polc:" + this.config.version + " --standard-json";
  ...
  downloadDockerImage(image) {
    ...
    try {
      execSync(`docker pull sophon/polc:${image}`);
      spinner.stop();
    } catch (error) {
      spinner.stop();
      throw new Error(error);
  ...
  async validateAndGetPolcVersion() {
    ...
    try {
      execSync("docker inspect --type=image sophon/polc:" + image);
    } catch (error) {
    ...
    const version = execSync(
      "docker run sophon/polc:" + image + " --version"
    );
    const normalized = new VersionRange().normalizePolcVersion(version);
