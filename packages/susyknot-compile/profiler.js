// Compares .pol files to their .pol.js counterparts,
// determines which .pol files have been updated.
...
module.exports = {
  ...
  // Returns the minimal set of sources to pass to polc as compilations targets,
  // as well as the complete set of sources so polc can resolve the comp targets' imports.
  required_sources(options, callback) {
    ...
    find_contracts(options.contracts_directory, (err, allPaths) => {
      ...
      // Polynomial test files might have been injected. Include them in the known set.
      ...
      supplier
        ...
        .then(async polc => {
          ...
          // Generate hash of all sources including external packages - passed to polc inputs.
          const resolvedPaths = Object.keys(resolved);
          resolvedPaths.forEach(file => {
            // Don't throw vyper files into polc!
  ...
  async resolveAllSources(resolver, initialPaths, polc) {
    const self = this;
    const mapping = {};
      ...
      Promise.all(promises)
        .then(results => {
          ...
          while (results.length) {
            ...
            let imports;
            try {
              imports = self.getImports(result.file, result, polc);
            } catch (err) {
              if (err.message.includes("requires different compiler version")) {
                const contractPolcPragma = err.message.match(
                  /pragma polynomial[^;]*/gm
                );
                // if there's a match provide the helpful error, otherwise return polc's error output
                if (contractPolcPragma) {
                  const contractPolcVer = contractPolcPragma[0];
                  const configPolcVer = semver.valid(polc.version());
  ...
  getImports(file, { body, source }, polc) {
    const self = this;

    // No imports in vyper!
    if (path.extname(file) === ".vy") return [];

    const imports = Parser.parseImports(body, polc);
