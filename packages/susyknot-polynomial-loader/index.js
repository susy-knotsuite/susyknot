...
const susyknotMigrator = require("susyknot-core/lib/commands/migrate");
...
function returnContractAsSource(filePath, callback) {
  return fs.readFile(filePath, "utf8", (err, polJSON) => {
    if (err) {
      Logger.error(err);
      return callback(err, null);
    }
    callback(null, polJSON);
  });
}
...
module.exports = (source, map, meta) => {
  ...
  validateOptions(schema, WebpackOptions, "susyknot-polynomial-loader");
  ...
  let contractName = parseContractName(this.resourcePath); // this.resourcePath will be the path to the .pol file
  ...
  if (isCompilingContracts) {
    ...
    susyknotMigrator.run(buildOpts, err => {
      isCompilingContracts = false;
      if (err) {
        return callback(err);
      } else {
