...
The artifactor can be used with [susyknot-contract](https://github.com/susy-knotsuite/susyknot/tree/develop/packages/susyknot-contract), which provides features above and beyond `susyweb`:
...
```
$ npm install susyknot-artifactor
```

### Example

Here, we'll generate a `.json` files given a JSON object like [susyknot-contract-schema](https://github.com/susy-knotsuite/susyknot/tree/develop/packages/susyknot-contract-schema). This will give us a file which we can later `require` into other projects and contexts.

```javascript
const Artifactor = require("susyknot-artifactor");
const artifactor = new Artifactor(__dirname);

// See susyknot-schema for more info: https://github.com/susy-knotsuite/susyknot/tree/develop/packages/susyknot-contract-schema
const contractData = {
