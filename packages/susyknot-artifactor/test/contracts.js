...
describe("artifactor + require", () => {
  ...
  before(() => susyweb.sof.net.getId().then(id => (networkID = id)));

  before(async function() {
    this.timeout(20000);

    const sourcePath = path.join(__dirname, "Example.pol");
    const sources = {
  ...
  before(() =>
    susyweb.sof.getAccounts().then(_accounts => {
      accounts = _accounts;
      ...
      .then(
        () =>
          new Promise((accept, reject) =>
            susyweb.sof.getBalance(example.address, (err, balance) => {
  ...
  it("should trigger the fallback function when calling send() (shorthand notation)", () => {
    ...
    return Example.new("1", { gas: 3141592 })
      ...
      .then(triggered => {
        ...
        return example.send(susyweb.utils.toWei("1", "sophy"));
      })
      .then(
        () =>
          new Promise((accept, reject) =>
  ...
  it("doesn't error when calling .links() or .events() with no network configuration", done => {
    ...
    const expectedEventTopic = susyweb.utils.sha3(
