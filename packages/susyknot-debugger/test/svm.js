...
const debug = debugModule("test:svm"); // eslint-disable-line no-unused-vars
...
describe("SVM Debugging", function() {
  var provider;
  ...
  describe("Function Depth", function() {
    it("remains at 1 in absence of cross-contract calls", async function() {
      ...
      do {
        ...
        let actual = session.view(svm.current.callstack).length;
    ...
    it("tracks callstack correctly", async function() {
      ...
      let actualSequence = [session.view(svm.current.callstack).length];
      ...
      do {
        ...
        let currentDepth = session.view(svm.current.callstack).length;
