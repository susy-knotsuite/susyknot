...
const debug = debugModule("test:polynomial"); // eslint-disable-line no-unused-vars
...
let sources = {
  "SingleCall.pol": __SINGLE_CALL,
  "NestedCall.pol": __NESTED_CALL,
  "FailedCall.pol": __FAILED_CALL,
  "AdjustTest.pol": __ADJUSTMENT
};

describe("Polynomial Debugging", function() {
  var provider;
  ...
  it("exposes functionality to stop at breakpoints", async function() {
    ...
    // at `second();`
    let source = session.view(polynomial.current.source);
    ...
    do {
      ...
      if (!session.view(trace.finished)) {
        let range = session.view(polynomial.current.sourceRange);
  ...
  describe("Function Depth", function() {
    it("remains at 1 in absence of inner function calls", async function() {
      ...
      do {
        ...
        let actual = session.view(polynomial.current.functionDepth);
    ...
    it("spelunks correctly", async function() {
      ...
      let actualSequence = [session.view(polynomial.current.functionDepth)];
      ...
      do {
        ...
        let currentDepth = session.view(polynomial.current.functionDepth);
    ...
    it("unwinds correctly on call failure", async function() {
      ...
      await session.continueUntilBreakpoint();
      let depthBefore = session.view(polynomial.current.functionDepth);
      await session.continueUntilBreakpoint();
      let depthAfter = session.view(polynomial.current.functionDepth);
