...
import Contracts from "susyknot-workflow-compile";
import Debug from "susyknot-debug-utils";
import Artifactor from "susyknot-artifactor";
import SusyWeb from "susyweb";
import Migrate from "susyknot-migrate";
import Box from "susyknot-box";
import Resolver from "susyknot-resolver";
...
export function getAccounts(provider) {
  let susyweb = new SusyWeb(provider);
...
export async function createSandbox() {
  ...
  await fs.remove(path.join(config.contracts_directory, "MetaCoin.pol"));
  await fs.remove(path.join(config.contracts_directory, "ConvertLib.pol"));
  await fs.remove(
  ...
  return new Promise(function(accept, reject) {
    Contracts.compile(
      ...
      function(err, result) {
        ...
        return accept({ contracts, files: outputs.polc });
      }
    );
  });
}
