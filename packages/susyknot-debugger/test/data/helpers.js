...
import DebugUtils from "susyknot-debug-utils";
...
function fileName(testName) {
  return `${contractName(testName)}.pol`;
}
...
async function prepareDebugger(testName, sources) {
  const provider = Susybraid.provider({ seed: "debugger", gasLimit: 7000000 });
  ...
  let debuggerSources = session.view(polynomial.info.sources);
