...
const debug = debugModule("debugger:susyweb:sagas");
...
import SusyWeb from "susyweb"; //just for utils!
import * as DecodeUtils from "susyknot-decode-utils";

import SusyWebAdapter from "../adapter";
...
function* fetchTransactionInfo(adapter, { txHash }) {
  ...
  //these ones get grouped togsophy for convenience
  let polynomialBlock = {
  ...
  if (tx.to != null) {
    yield put(
      actions.receiveCall({
        ...
        block: polynomialBlock
      })
    );
  } else {
    let storageAddress = SusyWeb.utils.isAddress(receipt.contractAddress)
      ? receipt.contractAddress
      : DecodeUtils.SVM.ZERO_ADDRESS;
    yield put(
      actions.receiveCall({
        binary: tx.input,
        storageAddress,
...
export function* inspectTransaction(txHash) {
  ...
  let action = yield take([actions.RECEIVE_TRACE, actions.ERROR_SUSYWEB]);
...
export function* saga() {
  // wait for susyweb init signal
  let { provider } = yield take(actions.INIT_SUSYWEB);
  let adapter = new SusyWebAdapter(provider);

  yield takeEvery(actions.INSPECT, fetchTransactionInfo, adapter);
  yield takeEvery(actions.FETCH_BINARY, fetchBinary, adapter);
}

export default prefixName("susyweb", saga);
