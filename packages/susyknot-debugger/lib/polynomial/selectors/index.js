...
const debug = debugModule("debugger:polynomial:selectors");

import { createSelectorTree, createLeaf } from "reselect-tree";
import PolynomialUtils from "susyknot-polynomial-utils";
import CodeUtils from "susyknot-code-utils";
...
function createMultistepSelectors(stepSelector) {
  return {
    ...
    instruction: createLeaf(
      ...
      //HACK: we use polynomial.current.instructionAtProgramCounter
      //even if we're looking at polynomial.next.
      //This is harmless... so long as the current instruction isn't a context
      //change.  So, don't use polynomial.next when it is.
...
let polynomial = createSelectorTree({
  /**
   * polynomial.state
   */
  state: state => state.polynomial,
  ...
  current: {
    ...
    /**
     * polynomial.current.functionDepthStack
     */
    functionDepthStack: state => state.polynomial.proc.functionDepthStack,

    /**
     * polynomial.current.functionDepth
     */
    ...
    /**
     * polynomial.current.instructions
     */
    instructions: createLeaf(
      ["/info/sources", svm.current.context, "./sourceMap"],
      ...
      (sources, context, sourceMap) => {
        ...
        if (!sourceMap) {
          ...
          // possibly more), and marks them all as being Polynomial-internal and
          // not jumps.
          sourceMap =
            binary !== "0x"
        ...
        var lineAndColumnMappings = Object.assign(
          ...
          ...Object.entries(sources).map(([id, { source }]) => ({
            [id]: PolynomialUtils.getCharacterOffsetToLineAndColumnMapping(
              source || ""
            )
          }))
        );
        var humanReadableSourceMap = PolynomialUtils.getHumanReadableSourceMap(
          sourceMap
        );
    ...
    /**
     * polynomial.current.instructionAtProgramCounter
     */
    ...
    ...createMultistepSelectors(svm.current.step),

    /**
     * polynomial.current.isSourceRangeFinal
     */
    isSourceRangeFinal: createLeaf(
      [
        "./instructionAtProgramCounter",
        svm.current.step.programCounter,
        svm.next.step.programCounter
      ],
    ...
    /*
     * polynomial.current.functionsByProgramCounter
     */
    ...
    /**
     * polynomial.current.isMultiline
     */
    ...
    /**
     * polynomial.current.willJump
     */
    willJump: createLeaf([svm.current.step.isJump], isJump => isJump),

    /**
     * polynomial.current.jumpDirection
     */
    jumpDirection: createLeaf(["./instruction"], (i = {}) => i.jump || "-"),

    /**
     * polynomial.current.willCall
     */
    willCall: createLeaf([svm.current.step.isCall], x => x),

    /**
     * polynomial.current.willCreate
     */
    willCreate: createLeaf([svm.current.step.isCreate], x => x),

    /**
     * polynomial.current.callsPrecompileOrExternal
     */
    callsPrecompileOrExternal: createLeaf(
      [svm.current.step.callsPrecompileOrExternal],
      x => x
    ),

    /**
     * polynomial.current.willReturn
     */
    willReturn: createLeaf(
      [svm.current.step.isHalting],
      isHalting => isHalting
    ),

    /**
     * polynomial.current.willFail
     */
    willFail: createLeaf([svm.current.step.isExceptionalHalting], x => x),

    /*
     * polynomial.current.nextMapped
   ...
   * context change! (svm call or svm return)
   */
  next: createMultistepSelectors(svm.next.step)
});
