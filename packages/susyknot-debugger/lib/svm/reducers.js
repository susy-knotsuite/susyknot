...
const debug = debugModule("debugger:svm:reducers");
...
const DEFAULT_TX = {
  gasprice: new BN(0),
  origin: DecodeUtils.SVM.ZERO_ADDRESS
};
...
const DEFAULT_BLOCK = {
  coinbase: DecodeUtils.SVM.ZERO_ADDRESS,
  difficulty: new BN(0),
  gaslimit: new BN(0),
  number: new BN(0),
  timestamp: new BN(0)
  ...
  switch (action.type) {
    ...
    case actions.CREATE:
      ...
      if (
        ...
        action.storageAddress === DecodeUtils.SVM.ZERO_ADDRESS
      ) {
        //if we don't
        return newState;
      }
    ...
    case actions.STORE: {
      ...
      if (address === DecodeUtils.SVM.ZERO_ADDRESS) {
