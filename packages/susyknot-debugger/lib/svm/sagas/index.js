...
const debug = debugModule("debugger:svm:sagas");
...
import svm from "../selectors";

import * as trace from "lib/trace/sagas";

/**
 * Adds SVM bytecode context
 *
...
export function* addInstance(address, binary) {
  let search = yield select(svm.info.binaries.search);
...
export function* begin({
  ...
  debug("codex: %O", yield select(svm.current.codex));
  if (address) {
...
export function* callstackAndCodexSaga() {
  if (yield select(svm.current.step.isExceptionalHalting)) {
    ...
    if (yield select(svm.current.step.callsPrecompileOrExternal)) {
      return;
    }

    let address = yield select(svm.current.step.callAddress);
    let data = yield select(svm.current.step.callData);

    debug("calling address %s", address);

    if (yield select(svm.current.step.isDelegateCallStrict)) {
      //if delegating, leave storageAddress, sender, and value the same
      let { storageAddress, sender, value } = yield select(svm.current.call);
      yield put(actions.call(address, data, storageAddress, sender, value));
    } else {
      //this branch covers CALL, CALLCODE, and STATICCALL
      let currentCall = yield select(svm.current.call);
      let storageAddress = (yield select(svm.current.step.isDelegateCallBroad))
        ? currentCall.storageAddress //for CALLCODE
        : address;
      let sender = currentCall.storageAddress; //not the code address!
      let value = yield select(svm.current.step.callValue); //0 if static
      yield put(actions.call(address, data, storageAddress, sender, value));
    }
  } else if (yield select(svm.current.step.isCreate)) {
    debug("got create");
    let binary = yield select(svm.current.step.createBinary);
    let createdAddress = yield select(svm.current.step.createdAddress);
    let value = yield select(svm.current.step.createValue);
    let sender = (yield select(svm.current.call)).storageAddress;
  ...
  } else if (yield select(svm.current.step.isHalting)) {
    debug("got return");

    let { binary, storageAddress } = yield select(svm.current.call);

    if (binary) {
      //if we're returning from a successful creation call, let's log the
      //result
      let returnedBinary = yield select(svm.current.step.returnValue);
      let search = yield select(svm.info.binaries.search);
  ...
  } else if (yield select(svm.current.step.touchesStorage)) {
    let storageAddress = (yield select(svm.current.call)).storageAddress;
    let slot = yield select(svm.current.step.storageAffected);
    //note we get next storage, since we're updating to that
    let storage = yield select(svm.next.state.storage);
    ...
    if (yield select(svm.current.step.isStore)) {
...
export function* reset() {
  let initialCall = yield select(svm.transaction.initialCall);
...
export default prefixName("svm", saga);
