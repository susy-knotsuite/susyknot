...
const debug = debugModule("debugger:svm:selectors"); // eslint-disable-line no-unused-vars
...
/**
 * create SVM-level selectors for a given trace step selector
...
const svm = createSelectorTree({
  /**
   * svm.state
   */
  state: state => state.svm,
  ...
  current: {
    /**
     * svm.current.callstack
     */
    callstack: state => state.svm.proc.callstack,
     ...
     * svm.current.state
     *
     * svm state info: as of last operation, before op defined in step
     */
    state: Object.assign(
      {},
    ...
    step: {
      ...
      /*
       * svm.current.step.createdAddress
       *
       ...
       * groups togsophy calls, creates, halts, and exceptional halts
       */
      ...
      /**
       * svm.current.step.isExceptionalHalting
       */
      ...
      returnStatus: createLeaf(
        ...
        (isHalting, { stack }, remaining, finalStatus) => {
          ...
          if (remaining <= 1) {
            ...
            const ZERO_WORD = "00".repeat(DecodeUtils.SVM.WORD_SIZE);
    ...
    /**
     * svm.current.codex (namespace)
     */
    codex: {
      /**
       * svm.current.codex (selector)
  ...
  next: {
     ...
     * svm.next.state
     *
     * svm state as a result of next step operation
     */
    state: Object.assign(
      {},
      ...["depth", "error", "gas", "memory", "stack", "storage"].map(param => ({
  ...
  nextOfSameDepth: {
    /**
     * svm.nextOfSameDepth.state
     *
     * svm state at the next step of same depth
     */
    state: Object.assign(
      {},
      ...["depth", "error", "gas", "memory", "stack", "storage"].map(param => ({
