...
export function* decode(definition, ref) {
  ...
  let ZERO_WORD = new Uint8Array(DecodeUtils.SVM.WORD_SIZE);
  ZERO_WORD.fill(0);
  let NO_CODE = new Uint8Array(); //empty array

  let decoder = forSvmState(definition, ref, {
  ...
  while (!result.done) {
    ...
    switch (request.type) {
      ...
      case "code":
        ...
        if (address in instances) {
        ...
        } else if (address === DecodeUtils.SVM.ZERO_ADDRESS) {
          ...
          //I don't want to write a new susyweb saga, so let's just use
          //obtainBinaries with a one-element array
          debug("fetching binary");
          let binary = (yield* susyweb.obtainBinaries([address], blockNumber))[0];
