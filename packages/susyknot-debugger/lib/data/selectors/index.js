...
import * as DecodeUtils from "susyknot-decode-utils";
...
const data = createSelectorTree({
  ...
  views: {
    ...
    scopes: {
      ...
      inlined: {
         ...
         */
        raw: createLeaf(
          ["/info/scopes/raw", polynomial.info.sources],
     ...
     */
    blockNumber: createLeaf([svm.transaction.globals.block], block =>
      block.number.toString()
    ),

    /*
     * data.views.instances
     * same as svm.current.codex.instances, but we just map address => binary,
     * we don't bother with context, and also the code is a Uint8Array
     */
    instances: createLeaf([svm.current.codex.instances], instances =>
      Object.assign(
        {},
     ...
     */
    contexts: createLeaf([svm.info.contexts], contexts =>
      Object.assign(
        {},
      ...
      _: createLeaf(["./raw", "/views/scopes/inlined/raw"], (scopes, inlined) =>
        Object.assign(
          ...
          ...Object.entries(scopes).map(([id, scope]) => {
            ...
            let newScope = { ...scope };
            //note that Polynomial gives us the linearization in order from most
  ...
  current: {
    ...
    state: {
       ...
       * I've named these after the polynomial variables they correspond to,
       * which are *mostly* the same as the corresponding SVM opcodes
       * (FWIW: this = ADDRESS, sender = CALLER, value = CALLVALUE)
       */
      specials: createLeaf(
        ["/current/address", svm.current.call, svm.transaction.globals],
     ...
     * data.current.node
     */
    node: createLeaf([polynomial.current.node], identity),
    ...
    functionDepth: createLeaf([polynomial.current.functionDepth], identity),
    ...
    address: createLeaf([svm.current.call], call => call.storageAddress),
     ...
     */
    context: createLeaf([svm.current.context], debuggerContextToDecoderContext),
    ...
    aboutToModify: createLeaf(
      [
        ...
        svm.current.step.isContextChange
      ],
  ...
  next: {
    ...
    //HACK WARNING
    //the following selectors depend on polynomial.next
    //do not use them when the current instruction is a context change!

    /**
     * data.next.node
     */
    node: createLeaf([polynomial.next.node], identity),
     ...
     */
    modifierInvocation: createLeaf(
      ["./node", "/views/scopes/inlined", svm.current.step.isContextChange],
      (node, scopes, invalid) => {
