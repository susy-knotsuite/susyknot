...
import { slotAddress } from "susyknot-decoder";
import { makeAssignment } from "lib/helpers";
import { Conversion, Definition, SVM } from "susyknot-decode-utils";
...
//a note on the following reducer: polynomial assigns a unique AST ID to every
//AST node among all the files being compiled togsophy.  thus, it is, for now,
  ...
  switch (action.type) {
    case actions.MAP_PATH_AND_ASSIGN:
      ...
      let parentAddress = slot.path
        ? Conversion.toHexString(slotAddress(slot.path), SVM.WORD_SIZE)
        : undefined;
