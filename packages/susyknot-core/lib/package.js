...
const SofPM = require("sofpm");
const SofPMRegistry = require("sofpm-registry");
...
const Package = {
  install: async function(options, callback) {
    expect.options(options, ["working_directory", "sofpm"]);

    expect.options(options.sofpm, ["registry", "ipfs_host"]);

    expect.one(options.sofpm, ["provider", "install_provider_uri"]);

    // ipfs_port and ipfs_protocol are optinal.

    const provider =
      options.sofpm.provider ||
      new SusyWeb.providers.HttpProvider(options.sofpm.install_provider_uri, {
        keepAlive: false
      });
    let host = options.sofpm.ipfs_host;

    if (host instanceof SofPM.hosts.IPFS === false) {
      host = new SofPM.hosts.IPFSWithLocalReader(
        options.sofpm.ipfs_host,
        options.sofpm.ipfs_port,
        options.sofpm.ipfs_protocol
      );
    }

    // When installing, we use infura to make a bunch of sof_call's.
    ...
    let registry = options.sofpm.registry;

    if (typeof registry === "string") {
      registry = await SofPMRegistry.use(
        options.sofpm.registry,
        fakeAddress,
        provider
      );
    }

    const pkg = new SofPM(options.working_directory, host, registry);
    ...
    if (options.packages) {
      ...
      fs.access(
        path.join(options.working_directory, "sofpm.json"),
        fs.constants.R_OK,
        error => {
          let manifest;

          // If the sofpm.json file doesn't exist, use the config as the manifest.
  ...
  publish: function(options, callback) {
    ...
    if (!ropsten) {
      return callback(
        new SusyknotError(
          "You need to have a `ropsten` network configured in order to publish to the Sophon Package Registry. See the following link for an example configuration:" +
            OS.EOL +
            OS.EOL +
            "    http://susyknotframework.com/tutorials/using-infura-custom-provider" +
            OS.EOL
        )
      );
    }
    ...
    var susyweb = new SusyWebShim({
      provider: options.provider,
      networkType: "sophon"
    });
    var host = options.sofpm.ipfs_host;

    if (host instanceof SofPM.hosts.IPFS === false) {
      host = new SofPM.hosts.IPFS(
        options.sofpm.ipfs_host,
        options.sofpm.ipfs_port,
        options.sofpm.ipfs_protocol
      );
    ...
    self.publishable_artifacts(options, function(err, artifacts) {
      ...
      susyweb.sof
        .getAccounts()
        .then(async accs => {
          var registry = await SofPMRegistry.use(
            options.sofpm.registry,
            accs[0],
            provider
          );
          var pkg = new SofPM(options.working_directory, host, registry);
    ...
    Networks.asURIs(options, deployed_networks, function(err, result) {
      ...
      if (!files.length) {
        var msg =
          ...
          ". " +
          "Run `susyknot compile` before publishing.";
