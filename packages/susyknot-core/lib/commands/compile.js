...
const command = {
  ...
  help: {
    usage: "susyknot compile [--list <filter>] [--all] [--network <name>]",
    options: [
      {
        option: "--all",
        description:
      ...
      },
      {
        option: "--list <filter>",
        description:
          "List all recent stable releases from polc-bin.  If filter is specified then it will display only " +
  ...
  listVersions: function(supplier, options, done) {
    ...
    if (options.list === "docker") {
      return supplier
        .getDockerTags()
        .then(tags => {
          tags.push("See more at: hub.docker.com/r/sophon/polc/tags/");
