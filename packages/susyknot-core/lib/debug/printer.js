...
const DebugUtils = require("susyknot-debug-utils");

const selectors = require("susyknot-debugger").selectors;
const { session, polynomial, trace, controller } = selectors;
...
class DebugPrinter {
  ...
  printFile() {
    ...
    const sourcePath = this.session.view(polynomial.current.source).sourcePath;
  ...
  printState() {
    const source = this.session.view(polynomial.current.source).source;
    const range = this.session.view(polynomial.current.sourceRange);
  ...
  printInstruction() {
    const instruction = this.session.view(polynomial.current.instruction);
  ...
  printBreakpoints() {
    let sourceNames = Object.assign(
      {},
      ...Object.values(this.session.view(polynomial.info.sources)).map(
        ({ id, sourcePath }) => ({
  ...
  async evalAndPrintExpression(raw, indent, suppress) {
    ...
    //using them is closer to the Polynomial syntax
