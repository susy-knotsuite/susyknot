...
var abi = require("susyweb-sof-abi");
        ...
        function(err, data) {
          ...
          abis.map(function(abi) {
            if (abi.type === "event") {
              ...
              self.known_events[self.susyweb.utils.sha3(signature)] = {
                signature: signature,
                abi_entry: abi
              };
            }
...
TestRunner.prototype.startTest = function(mocha, callback) {
  ...
  this.susyweb.sof
    .getBlockNumber()
    .then(result => {
      var one = self.susyweb.utils.toBN(1);
      result = self.susyweb.utils.toBN(result);
...
TestRunner.prototype.endTest = function(mocha, callback) {
  ...
  // There's no API for sof_getLogs?
  this.rpc(
    "sof_getLogs",
    [
      {
...
(TestRunner.prototype.snapshot = function(callback) {
  this.rpc("svm_snapshot", function(err, result) {
    if (err) return callback(err);
    callback(null, result.result);
  });
}),
  (TestRunner.prototype.revert = function(snapshot_id, callback) {
    this.rpc("svm_revert", [snapshot_id], callback);
  });
