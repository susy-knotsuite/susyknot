...
var find_contracts = require("susyknot-contract-sources");
...
TestSource.prototype.resolve = function(import_path, callback) {
  ...
  if (import_path === "susyknot/DeployedAddresses.pol") {
    return find_contracts(this.config.contracts_directory, function(
      ...
      fs.readdir(self.config.contracts_build_directory, function(
        ...
        source_files.forEach(function(file) {
          var name = path.basename(file, ".pol");
        ...
        Promise.all(promises)
          .then(function(files_data) {
            ...
            return Deployed.makePolynomialDeployedAddressesLibrary(
              mapping,
  ...
  for (const lib of assertLibraries) {
    if (import_path === `susyknot/${lib}.pol`)
      return fs.readFile(
        path.resolve(path.join(__dirname, `${lib}.pol`)),
        { encoding: "utf8" },
