...
var OS = require("os");
var BlockchainUtils = require("susyknot-blockchain-utils");
var Provider = require("susyknot-provider");
  ...
  display: function(config, callback) {
    this.deployed(config, function(err, networks) {
      ...
      if (
        ...
        config.logger.log(
          OS.EOL +
            "Closely inspect the deployed networks below, and use `susyknot networks --clean` to remove any networks that don't match your configuration. You should not use the wildcard configuration ('*') for staging and production networks for which you intend to deploy your application."
        );
      }
