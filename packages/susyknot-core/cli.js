...
if (!semver.satisfies(process.version, ">=" + minimumNodeVersion)) {
  console.log(
    "Error: Node version not supported. You are currently using version " +
      ...
      " of Node. Susyknot requires Node v" +
      minimumNodeVersion +
      " or higher."
  );
...
// https://octonion.institute/susy-js/susyweb.js/issues/1648
...
command.run(inputArguments, options, function(err) {
  if (err) {
    if (err instanceof TaskError) {
      ...
      if (err instanceof SusyknotError) {
        analytics.send({
          exception: "SusyknotError - missing configuration file",
          version: versionInfo.bundle
            ? versionInfo.bundle
            : "(unbundled) " + versionInfo.core
        });
        console.log(err.message);
        version.logSusyknotAndNode(options.logger);
        ...
        if (error === err.stack) {
          ...
          //analytics by cutting off everything before susyknot. Will not properly catch the user's info
          //here if the user has susyknot in their name.
          let identifyingInfo = String.raw`${directory.split("susyknot")[0]}`;
