...
describe("migrate", function() {
  ...
  function createProviderAndSetNetworkConfig(network) {
    var provider = Susybraid.provider({ seed: network, gasLimit: config.gas });
    var susyweb = new SusyWeb(provider);
    return susyweb.sof.getAccounts().then(accs => {
      return susyweb.sof.net.getId().then(network_id => {
