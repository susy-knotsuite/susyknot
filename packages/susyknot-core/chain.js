...
// to be parsed and then passed to Susybraid.server().
var ipcNetwork;
var options;
...
try {
  ...
  throw new Error(
    "Fatal: Error parsing arguments; please contact the Susyknot developers for help."
  );
}
...
options.default_sophy_balance = options.default_sophy_balance || 100;
...
// constructor - accepts options for Susybraid
function SusybraidMixin(options) {
  this.susybraid = Susybraid.server(options);
}

// start Susybraid and capture promise that resolves when ready
SusybraidMixin.prototype.start = function(supervisor) {
  var self = this;

  this.ready = new Promise(function(accept, reject) {
    self.susybraid.listen(options.port, options.hostname, function(err, state) {
      if (err) {
        reject(err);
      }
...
// wait for Susybraid to be ready then emit signal to client socket
SusybraidMixin.prototype.connect = function(supervisor, socket) {
  var self = this;
  this.ready.then(function() {
    supervisor.emit(socket, "susyknot.ready");
  });
};

// cleanup Susybraid process on exit
SusybraidMixin.prototype.exit = function(supervisor) {
  this.susybraid.close(function(err) {
    if (err) {
...
// constructor - takes Logger instance and message key (e.g. `susyknot.ipc.log`)
...
var susybraidLogger = new Logger();
...
options.logger = { log: susybraidLogger.log.bind(susybraidLogger) };

supervisor.use(new LifecycleMixin());
supervisor.use(new SusybraidMixin(options));
supervisor.use(new LoggerMixin(ipcLogger, "susyknot.ipc.log"));
supervisor.use(new LoggerMixin(susybraidLogger, "susyknot.susybraid.log"));
supervisor.start();
