...
let SusyWeb = require("susyweb");
const webUtils = require("susyweb-utils");
...
if (typeof SusyWeb === "object" && Object.keys(SusyWeb).length === 0) {
  SusyWeb = global.SusyWeb;
}

(function(module) {
  // Accepts a contract object created with susyweb.sof.Contract or an address.
  function Contract(contract) {
    ...
    if (typeof contract === "string") {
      var susywebInstance = new constructor.susyweb.sof.Contract(constructor.abi);
      susywebInstance.options.address = contract;
      contract = susywebInstance;
    }
    ...
    instance.abi.forEach(function(item) {
      switch (item.type) {
        case "function":
          ...
          var method = function(constant, susywebMethod) {
            var fn;
