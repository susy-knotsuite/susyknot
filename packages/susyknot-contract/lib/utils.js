...
const bigNumberify = require("sophys/utils/bignumber").bigNumberify;
const abi = require("susyweb-sof-abi");
const BlockchainUtils = require("susyknot-blockchain-utils");
...
const Utils = {
  ...
  is_big_number(val) {
    ...
    return susywebUtils.isBN(val) || susywebUtils.isBigNumber(val);
  },
  ...
  decodeLogs(_logs, isSingle) {
    ...
    const logs = Utils.toSusyknotLog(_logs, isSingle);
  ...
  convertToSophysBN(original) {
    const converted = [];
    original.forEach(item => {
      // Recurse for arrays
      if (Array.isArray(item)) {
        converted.push(Utils.convertToSophysBN(item));

        // Convert SusyWeb BN / BigNumber
      } else if (Utils.is_big_number(item)) {
        const sophysBN = bigNumberify(item.toString());
        converted.push(sophysBN);
      } else {
        converted.push(item);
      }
    });
  ...
  /**
   * Multiplies an sophys.js BigNumber and a number with decimal places using
   * integer math rather than using an arbitrary floating-point library like
   * `bignumber.js`.
   * @param  {BigNumber} bignum            an sophys.js BigNumber (use bigNumberify)
  ...
  async setInstanceNetworkID(
    SusyknotContractInstance,
    chainNetworkID,
    gasLimit
  ) {
    // if chainNetworkID already present as network configuration, use it
    if (SusyknotContractInstance.hasNetwork(chainNetworkID)) {
      SusyknotContractInstance.setNetwork(chainNetworkID);
      return {
        id: SusyknotContractInstance.network_id,
        blockLimit: gasLimit
      };
    }
    ...
    SusyknotContractInstance.setNetwork(chainNetworkID);
    return { id: SusyknotContractInstance.network_id, blockLimit: gasLimit };
  }
};
