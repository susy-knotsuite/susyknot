const SusyWebPromiEvent = require("susyweb-core-promievent");
...
module.exports = Contract => ({
  setProvider(provider) {
    ...
    this.susyweb.setProvider(provider);
    this.currentProvider = provider;
  },

  new() {
    const promiEvent = new SusyWebPromiEvent();
  ...
  async at(address) {
    ...
    try {
      ...
      const onChainCode = await this.susyweb.sof.getCode(address);
  ...
  async detectNetwork() {
    ...
    if (this.network_id && this.networks[this.network_id] != null) {
      try {
        const { gasLimit } = await this.susyweb.sof.getBlock("latest");
    ...
    try {
      const chainNetworkID = await this.susyweb.sof.net.getId();
      const { gasLimit } = await this.susyweb.sof.getBlock("latest");
  ...
  setNetworkType(networkType) {
    if (this.susyweb) {
      this.susyweb.setNetworkType(networkType);
    }
  ...
  setWallet(wallet) {
    this.susyweb.sof.accounts.wallet = wallet;
  },
  ...
  clone(json) {
    ...
    const temp = function SusyknotContract() {
