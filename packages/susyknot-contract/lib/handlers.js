...
var handlers = {
  ...
  maxConfirmations: 24, // Maximum number of confirmation susyweb emits
  defaultTimeoutBlocks: 50, // Maximum number of blocks susyweb will wait before abandoning tx
  timeoutMessage: "50 blocks", // Substring of susyweb timeout error.
  ...
  /**
   * Attaches Susyknot specific handlers to all of the events emitted by a susyweb method.
   * @param {Object}       context  execution state
   * @param {PromiEvent}   emitter  promiEvent returned by a susyweb method call
   */
  ...
  confirmation: function(context, number, receipt) {
    ...
    // Per susyweb: initial confirmation index is 0
  ...
  receipt: async function(context, receipt) {
    ...
    if (receipt.status !== undefined && !receipt.status) {
      var reason = await Reason.get(context.params, context.contract.susyweb);
