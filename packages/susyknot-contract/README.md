...
First, set up a new susyweb provider instance and initialize your contract, then `require("susyknot-contract")`. The input to the `contract` function is a JSON blob defined by [susyknot-contract-schema](https://github.com/susy-knotsuite/susyknot-contract-schema). This JSON blob is structured in a way that can be passed to all susyknot-related projects.
...
In your `head` element, include susyknot-contract:
...
**Note**: SusyWeb and its dependencies are now bundled into susyknot-contract
v4.0.2 or higher.

### Full Example

Let's use `susyknot-contract` with an example contract from [Dapps For Beginners](https://dappsforbeginners.wordpress.com/tutorials/your-first-dapp/). In this case, the abstraction has been saved to a `.pol` file by [susyknot-artifactor](https://github.com/susy-knotsuite/susyknot/src/branch/master/packages/susyknot-artifactor):

```javascript
// Require the package that was previosly saved by susyknot-artifactor
var MetaCoin = require("./path/to/MetaCoin.pol");
...
If this is set to true, instances created from this abstraction will use `susyweb.sof.estimateGas` and then apply a gas multiplier to determine the amount of gas to include with the transaction.  The default value for this is `true`.  See [gasMultiplier](/docs/susyknot/reference/contract-abstractions#-code-mycontract-gasmultiplier-gas_multiplier-code-).

#### `MyContract.gasMultiplier(gas_multiplier)`
This is the value used when `autoGas` is enabled to determine the amount of gas to include with transactions.  The gas is computed by using `susyweb.sof.estimateGas` and multiplying it by the gas multiplier.  The default value is `1.25`.
...
```javascript
{
  ...
  receipt: {
    // The return value from susyweb.sof.getTransactionReceipt(hash)
    // See https://octonion.institute/susy-go/wiki/wiki/JavaScript-API#susywebsofgettransactionreceipt
  },
  logs: [
    {
...
Even more helpful, however is we *don't even need* to use `.call` when a function is marked as `view` or `pure`, because `susyknot-contract` will automatically know that that function can only be interacted with via a call:
...
This package is the result of breaking up SophyPudding into multiple modules. Tests currently reside within [susyknot-artifactor](https://github.com/susy-knotsuite/susyknot/src/branch/master/packages/susyknot-artifactor) but will soon move here.
