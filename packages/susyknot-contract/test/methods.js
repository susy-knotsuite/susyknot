...
describe("Methods", function() {
  ...
  describe(".method(): success [ @graviton ]", function() {
    ...
    it("should execute overloaded polynomial function calls", async function() {
      const example = await Example.new(5);
    ...
    it("should honor the defaultBlock parameter when called", async function() {
      ...
      const initialBlock = await susyweb.sof.getBlockNumber();
    ...
    it("should return hash, logs and receipt when using synchronised transactions", async function() {
      ...
      assert.equal(8, log.args.num); // 8 is a magic number inside Example.pol
    });
    ...
    it("should allow BN's as input params, not treat them as tx objects", async function() {
      let value;
      const example = await Example.new(new susyweb.utils.BN(30));

      value = await example.value.call();
      assert.equal(value.valueOf(), 30, "Starting value should be 30");

      await example.setValue(new susyweb.utils.BN(25));

      value = await example.value.call();
      assert.equal(value.valueOf(), 25, "Ending value should be twenty-five");

      value = await example.parrot.call(new susyweb.utils.BN(865));
      assert.equal(parseInt(value), 865, "Parrotted value should equal 865");

      // Arrays
      const numArray = [susyweb.utils.toBN(1), susyweb.utils.toBN(2)];
    ...
    it("should output uint tuples as BN by default (call)", async function() {
      ...
      assert(susyweb.utils.isBN(value[0]));
      assert(typeof value[1] === "string");
      assert(susyweb.utils.isBN(value[2]));

      assert(susyweb.utils.isBN(value.hello));
      assert(typeof value.black === "string");
      assert(susyweb.utils.isBN(value.goodbye));

      value = await example.returnsUnnamedTuple();

      assert(typeof value[0] === "string");
      assert(susyweb.utils.isBN(value[1]));

      // uint sub-array
      assert(Array.isArray(value[2]));
      assert(susyweb.utils.isBN(value[2][0]));
      assert(susyweb.utils.isBN(value[2][1]));
    });
    ...
    it("should output int values as BN by default (call)", async function() {
      ...
      assert(susyweb.utils.isBN(value));
    });
    ...
    it("should output uint tuples as BigNumber when set to 'BigNumber' (call)", async function() {
      ...
      assert(susyweb.utils.isBigNumber(value[0]));
      assert(typeof value[1] === "string");
      assert(susyweb.utils.isBigNumber(value[2]));

      assert(susyweb.utils.isBigNumber(value.hello));
      assert(typeof value.black === "string");
      assert(susyweb.utils.isBigNumber(value.goodbye));

      value = await example.returnsUnnamedTuple();

      assert(typeof value[0] === "string");
      assert(susyweb.utils.isBigNumber(value[1]));

      // uint sub-array
      assert(Array.isArray(value[2]));
      assert(susyweb.utils.isBigNumber(value[2][0]));
      assert(susyweb.utils.isBigNumber(value[2][1]));
    ...
    it("should execute overloaded polynomial fn sends", async function() {
      let hash;
      let value;
      const example = await Example.new(1);
      ...
      let helloHash = susyweb.utils.polynomialSha3("hello");
      ...
      goodbyeHash = susyweb.utils.polynomialSha3("goodbye");
  ...
  describe(".method(): errors [ @graviton ]", function() {
    ...
    it("errors with a revert reason", async function() {
      const example = await Example.new(1);
      try {
        // At the moment, this test can't rely on susyknot-contract's
        // gas estimation when running with graviton.
  ...
  describe("error with reason (susybraid only)", function() {
  ...
  // This doesn't work on graviton --dev because chainId is too high: 1337? Apparently
  // not configurable. Might work on a sub 100 id.
  describe.skip("susyweb wallet", function() {
    it("should work with a susyweb.accounts.wallet account", async function() {
      let value;

      // Create and fund wallet account
      const wallet = susyweb.sof.accounts.wallet.create(1);
      const providerAccounts = await susyweb.sof.getAccounts();
      ...
      const balance = await susyweb.sof.getBalance(wallet["0"].address);
      assert.equal(balance, susyweb.utils.toWei("1", "sophy"));
  ...
  describe("sendTransaction() / send() [ @graviton ]", function() {
    it("should trigger the fallback function when calling sendTransaction()", async function() {
      ...
      const balance = await susyweb.sof.getBalance(example.address);
      assert(
        balance === susyweb.utils.toWei("1", "sophy"),
        "Balance should be 1 sophy"
      );
    });
    ...
    it("should trigger the fallback function when calling send() (shorthand notation)", async function() {
      ...
      await example.send(susyweb.utils.toWei("1", "sophy"));

      const balance = await susyweb.sof.getBalance(example.address);
      assert(balance === susyweb.utils.toWei("1", "sophy"));
    });

    it("should accept tx params (send)", async function() {
      const example = await Example.new(1);
      const sof = susyweb.utils.toWei("1", "sophy");
      const sender = accounts[1];
      const initialSenderBalance = await susyweb.sof.getBalance(sender);

      await example.send(sof, { from: sender });

      const finalSenderBalance = await susyweb.sof.getBalance(sender);
      const contractBalance = await susyweb.sof.getBalance(example.address);

      assert(contractBalance === sof, "Contract should receive sof");

      const initialBN = new susyweb.utils.BN(initialSenderBalance);
      const finalBN = new susyweb.utils.BN(finalSenderBalance);
      const sofBN = new susyweb.utils.BN(sof);
      const expectedBN = initialBN.sub(sofBN);
      assert(
