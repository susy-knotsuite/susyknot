  ...
  it('should expose the "on" handler / format event correctly', function(done) {
    Example.new(1).then(example => {
      ...
      event.on("data", function(data) {
        ...
        assert.equal(8, data.args.num); // 8 is a magic number inside Example.pol
  ...
  it("should reformat numbers in events to BN by default", function(done) {
    Example.new(1).then(example => {
      ...
      event.once("data", function(data) {
        ...
        assert(susyweb.utils.isBN(args[0])); // int named
        assert(susyweb.utils.isBN(args[1])); // int named, indexed

        assert(!susyweb.utils.isBN(args[2])); // Address

        assert(susyweb.utils.isBN(args[3])); // uint named
        assert(susyweb.utils.isBN(args[4])); // uint unnamed

        assert(susyweb.utils.isBN(args.numA));
        assert(susyweb.utils.isBN(args.numB));

        assert(!susyweb.utils.isBN(args.addressC));

        assert(susyweb.utils.isBN(args.numD));
