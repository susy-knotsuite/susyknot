...
describe("Different networks: ", function() {
  ...
  before("Compile", async function() {
    ...
    network_one_id = 1000;
    network_two_id = 1001;
    network_one = Susybraid.provider({
      network_id: network_one_id,
      seed: network_one_id,
      logger: log
    });
    network_two = Susybraid.provider({
  ...
  it("does not deploy to the same network (sof_getCode)", function(done) {
    function getCode(firstContract, secondContract, callback) {
      firstContract.susyweb.sof.getCode(secondContract.address, callback);
    }
  ...
  it("deployed() used as a thennable will error if contract hasn't been deployed to the network detected", function(done) {
    var network_three = Susybraid.provider();
