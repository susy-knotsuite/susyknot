...
describe("HD Wallet Provider", function() {
  ...
  it("provides for a mnemonic", function(done) {
    ...
    assert.deepEqual(provider.getAddresses(), susyknotDevAccounts);
    susyweb.setProvider(provider);

    susyweb.sof.getBlockNumber((err, number) => {
      assert(number === 0);
      done();
    });
  });
  ...
  it("provides for a private key", function(done) {
    const privateKey =
      "3f841bf589fdf83a521e55d51afddc34fa65351161eead24f064855fc29c9580"; //random valid private key generated with sofkey
    ...
    addresses.forEach(address => {
      assert(SofUtil.isValidAddress(address), "invalid address");
    });
  ...
  it("provides for an array of private keys", function(done) {
    ...
    addresses.forEach(address => {
      ...
      const expectedAddress = `0x${SofUtil.privateToAddress(
        privateKey
      ).toString("hex")}`;
      assert.equal(
        address,
