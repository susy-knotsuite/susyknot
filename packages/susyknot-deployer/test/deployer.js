...
const Reporter = require("susyknot-reporters").migrationsV5;
...
describe("Deployer (sync)", function() {
  ...
  // There's a chain like this in the susyknot-core polynomial-tests
  ...
  it("waits for confirmations", async function() {
    this.timeout(15000);
    const startBlock = await susyweb.sof.getBlockNumber();

    utils.startAutoMine(susyweb, 1500);
    ...
    const libReceipt = await susyweb.sof.getTransactionReceipt(
      IsLibrary.transactionHash
    );
    const exampleReceipt = await susyweb.sof.getTransactionReceipt(
  ...
  it("emits block events while waiting for a tx to mine", async function() {
    ...
    utils.startAutoMine(susyweb, 4000);

    const migrate = function() {
      deployer.then(async function() {
        await deployer._startBlockPolling(susyweb);
