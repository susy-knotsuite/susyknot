...
class Deployment {
   ...
   */
  async _startBlockPolling(susyweb) {
    const self = this;
    const startTime = new Date().getTime();

    let secondsWaited = 0;
    let blocksWaited = 0;
    let currentBlock = await susyweb.sof.getBlockNumber();

    self.blockPoll = setInterval(async () => {
      const newBlock = await susyweb.sof.getBlockNumber();
   ...
   */
  async _waitBlocks(blocksToWait, state, susyweb) {
    const self = this;
    let currentBlock = await susyweb.sof.getBlockNumber();
   ...
   * @private
   * @param  {Object} contract SusyknotContract
  ...
  /**
   * There are outstanding issues at both graviton (with websockets) & susyweb (with confirmation handling
  ...
  executeDeployment(contract, args) {
    ...
    return async function() {
      ...
      const currentBlock = await contract.susyweb.sof.getBlock("latest");
      ...
      if (shouldDeploy) {
        /*
          ...
          susyknot-contract will defer to susyweb's defaults:
        ...
        */
        ...
        // NB: susyweb does not strip the revert msg here like it does for `deploy`
        try {
        ...
        await self._startBlockPolling(contract.susyweb);
      ...
      if (self.confirmations !== 0 && shouldDeploy) {
        await self._waitBlocks(self.confirmations, state, contract.susyweb);
      }
